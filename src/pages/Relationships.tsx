...
import { supabase } from '../lib/supabase';
import toast from 'react-hot-toast';
import { AddRelationshipModal } from '../components/RelationshipFormAndModal';
...
const RelationshipCard: React.FC<{
  ...
  const scoreColor = getScoreColor(relationship.latest_score);
  const navigate = useNavigate();
      ...
      <div className='p-6'>
              ...
              <span className='inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800'>
                {
                  RELATIONSHIP_TYPES.find(
                    (t) => t.value === relationship.relationship_type
                  )?.label
                }
              </span>
              <span className='text-gray-500'>
                {relationship.specific_role}
              </span>
            </div>
          </div>
          <button
        ...
        <div className='grid md:grid-cols-3 gap-6 mt-6'>
          ...
          <MetricCard
            ...
            value={`${relationship.completed_goals_count} of ${relationship.goals_count} completed`}
          ...
          {!relationship.latest_score && (
            <div className='flex items-center space-x-3'>
              ...
              <div>
                ...
                <button
                  onClick={() =>
                    navigate(`/relationships/${relationship.id}/assess`)
                  }
                  className='text-rose-500 hover:text-rose-600 font-medium'
                >
                  Take First Assessment
...
const MetricCard: React.FC<{
  ...
  value: string;
  className?: string;
}> = ({ icon, title, value, className = 'bg-gray-100 text-gray-600' }) => (
  <div className='flex items-center space-x-3'>
...
const Relationships = () => {
    ...
    try {
      ...
      const processedData = data.map((relationship) => ({
        ...
        latest_assessment_date:
          relationship.relationship_assessments?.[0]?.taken_at,
        goals_count: relationship.relationship_goals?.length || 0,
        completed_goals_count:
          relationship.relationship_goals?.filter(
  ...
  const handleAddRelationship = async (formData: RelationshipFormData) => {
    try {
      const {
        data: { user },
      } = await supabase.auth.getUser();
