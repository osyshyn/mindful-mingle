...
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { Sparkles, Calendar, BookOpen, Filter, Loader } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { Tip } from '../interfaces/interfaces';
import toast from 'react-hot-toast';
  ...
  return (
    <div className='max-w-6xl mx-auto'>
      <div className='text-center mb-8'>
        ...
        <h1 className='text-3xl font-bold text-gray-900'>Daily EQ Tips</h1>
        <p className='text-gray-600 mt-2'>
          Enhance your emotional intelligence with daily insights and practical
          advice
        </p>
      </div>
      ...
      <div className='relative flex items-stretch gap-6 min-h-[400px] mb-8'>
        {yesterdayTip && (
          <div
            className='w-1/4 bg-white rounded-xl shadow-md overflow-hidden opacity-75 hover:opacity-100 transition-opacity cursor-pointer transform hover:scale-105'
            onClick={() => handleTipClick(yesterdayTip)}
          >
            <div className='p-4'>
              <div className='flex items-center justify-between mb-3'>
        ...
        {todayTip && (
          ...
          >
            ...
            <div className='p-6'>
              ...
              <p className='text-lg text-gray-600 mb-4'>{todayTip.content}</p>
              <div className='flex items-center text-rose-500'>
                <BookOpen className='h-5 w-5 mr-2' />
                <span className='text-sm font-medium'>Read full article</span>
              </div>
          ...
          {filteredTips.slice(2).map((tip) => (
            ...
            >
              <div className='p-6'>
                ...
                <p className='text-gray-600 line-clamp-3'>{tip.content}</p>
                <div className='mt-4 flex items-center text-rose-500 hover:text-rose-600'>
                  <BookOpen className='h-5 w-5 mr-2' />
                  <span className='text-sm font-medium'>Read article</span>
                </div>
