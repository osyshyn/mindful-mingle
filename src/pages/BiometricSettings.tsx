...
import { supabase } from '../lib/supabase';
import toast from 'react-hot-toast';
import { BiometricProviderCard } from '../components/BiometricProviderCard';
import { biometricProviders } from '../lib/biometricProviders';
...
const BiometricSettings = () => {
  ...
  const handleConnect = async (providerId: string) => {
    ...
    try {console.log('Connecting:', providerId, 'for user:', user?.id);
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          settings: {
            sync_frequency: 'hourly',
            metrics: biometricProviders.find((p) => p.id === providerId)
              ?.metrics,
          },
        })
        .select()
      ...
      toast.success(
        `Connected to ${
          biometricProviders.find((p) => p.id === providerId)?.name
        }`
      );
    } catch {
      toast.error('Error connecting to provider');
  ...
  const handleDisconnect = async (providerId: string) => {
    try {
      ...
      setConnections((prev) =>
        prev.filter((conn) => conn.provider !== providerId)
      );
      toast.success(
        `Disconnected from ${
          biometricProviders.find((p) => p.id === providerId)?.name
    ...
    <div className='max-w-4xl mx-auto'>
      <div className='bg-white rounded-xl shadow-md overflow-hidden'>
        <div className='p-6 bg-rose-50 border-b border-rose-100'>
          ...
          <p className='text-gray-600 mt-2'>
            Connect your devices and apps to track stress levels and emotional
            states
          </p>
        </div>
